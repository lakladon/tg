...
🛍️ Магазин — стабильный поток клиентов, низкий риск
🏨 Отель — доход зависит от сезона, средний риск
🚧 Строительная компания — крупные контракты, высокий риск
🛒 Онлайн-магазин — быстрый рост, средний риск
🏋️ Фитнес-клуб — подписки и услуги, низкий риск
🛍️ Магазин — стабильный поток клиентов, низкий риск
🏨 Отель — доход зависит от сезона, средний риск
🚧 Строительная компания — крупные контракты, высокий риск
🛒 Онлайн-магазин — быстрый рост, средний риск
...
async def process_business_name(message: types.Message, state: FSMContext):
    ...
    if business_id:
        ...
        # Перечитываем игрока после обновления баланса; если None, подстрахуемся нулевыми значениями
        player = db.get_player(user_id) or {"balance": 0}
